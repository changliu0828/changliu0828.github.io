...
非对称协程以协程调用栈的方式运作，初始时栈内只有主协程。调用`resume`唤起其他协程入栈，并切换至其上下文运行。在协程运行完毕，或显示调用`yield`时，协程出栈，切换至上一个协程上下文运行。通常情况下，协程的调用栈不会很深，大多使用上以主协程（IO）与逻辑协程相互切换。

而对称协程中，各个协程平等运行，调用`transfer`在各个协程之间自由切换跳转。
